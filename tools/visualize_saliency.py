...
def main():
    ...
    for now, data_objs in enumerate(test_dataloader):
        ...
        for obj_idx, end_points in enumerate(data_objs):
            ...
            print("cloud saliency stats")
            print(np.min(dcloud_c), np.max(dcloud_c), np.mean(dcloud_c), np.std(dcloud_c))
            print(np.min(dcloud_r), np.max(dcloud_r), np.mean(dcloud_r), np.std(dcloud_r))
            print(np.min(dcloud_t), np.max(dcloud_t), np.mean(dcloud_t), np.std(dcloud_t))
